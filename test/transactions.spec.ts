...
describe('Transactions routes', () => {
    ...
    it('should be able to list all transactions', async () => {
        const createTransationResponse = await request(app.server).post('/transactions').send({
            title: 'New transaction',
            amount: 5000,
            type: 'credit'
        })
        ...
        const listTranscationsResponse = await request(app.server).get('/transactions').set('Cookie',cookies).expect(200)

        expect(listTranscationsResponse.body.transactions).toEqual([
    ...
    it('should be able to get specific transactions', async () => {
        ...
        const transactionId = listTranscationsResponse.body.transactions[0].id
